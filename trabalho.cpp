...
cout <<"Data de pagamento: "<< this->data_pagamento.substr(0,2) << "/" << this->data_pagamento.substr(2,2) << "/" << this->data_pagamento.substr(4,4) 
<<endl;
...
No* inserir(int codigo, string nome, int idade, string CPF, float divida, string data_pagamento, No* A) {
if(A == NULL) {
A = new No;
A->codigo = codigo;
A->nome=nome;
...
// FS
void exibe_em_atraso(No* A) {
if(A == NULL)
return;

//stoi(A->data_pagamento.substr(4,4)) ANO
//stoi(A->data_pagamento.substr(2,2)) MES 
//stoi(A->data_pagamento.substr(0,2)) DIA

exibe_em_atraso(A->esq);
if ((stoi(A->data_pagamento.substr(4,4)) == 2020
&& stoi(A->data_pagamento.substr(2,2)) >= 9 ) || 
(stoi(A->data_pagamento.substr(0,2)) >= 2020
)) {
A->exibir_no();
cout<<endl;
}
exibe_em_atraso(A->dir);
...
void inserir_interface(int codigo, string nome, int idade, string CPF, float divida, string data_pagamento) {
...
// FS
void exibir_a_maior_divida() {
cout <<"Codigo: "<< this->maior_divida->codigo <<endl;
cout <<"Nome: "<< this->maior_divida->nome <<endl;
cout <<"CPF: "<< this->maior_divida->CPF <<endl;
cout <<"Dívida: "<< this->maior_divida->divida <<endl;

cout << endl;
}

void exibir_dividas_em_atraso() {
exibe_em_atraso(raiz);
cout << endl;
}
